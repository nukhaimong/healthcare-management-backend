...
const getDashboardStatsData = async (user: IRequestUser) => {
  ...
  switch (user.role) {
    ...
    default:
      throw new AppError(status.BAD_REQUEST, 'Ivalid user role');
  }
  return statsData;
};
...
const getPatientStatsData = async (user: IRequestUser) => {
  ...
  const appointCount = await prisma.appointment.count({
    where: {
  ...
  const formattedAppointmetnStatusDistribution =
    appointmentStatusDistribution.map(({ _count, status }) => ({
      status,
      count: _count.id,
    }));
  ...
  return {
    appointCount,
    reviewCount,
    appointmentStatusDistribution: formattedAppointmetnStatusDistribution,
  };
};
