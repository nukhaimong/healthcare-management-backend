...
const givePrescription = async (
  ...
  if (appointmentData.doctorId !== doctorData.id) {
    throw new AppError(
      status.BAD_REQUEST,
      'You can only give prescription for your own prescription',
    );
  }

  console.log('appoinment id: ', payload.appointmentId);
  ...
  const result = await prisma.$transaction(
    async (tx) => {
      ...
      const uploadFile = await uploadFileCloudinary(pdfBuffer, fileName);
...
const updatePrescription = async (
  ...
  const uploadedFile = await uploadFileCloudinary(pdfBuffer, fileName);
