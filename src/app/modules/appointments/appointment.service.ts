...
import { stripe } from '../../../config/stripe.congfig';
...
const bookAppointment = async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${envVars.FRONTEND_URL}/dashboard/appointments/payment-success?appointment_id=${appointmentData.id}&payment_id=${paymentData.id}`,
      cancel_url: `${envVars.FRONTEND_URL}/dashboard/appoitments?error=payment_cancelled`,
    });
    return {
      appointmentData,
      paymentData,
...
/*
  ...
  2. Doctors can only update appointment status form schedule to inprogress pr inprogress to completed complted or schedule to cancelled.
...
const cancelUnpaidAppointments = async () => {
  const thirtyMinutesAgo = new Date(Date.now() - 30 * 60 * 100);
